...
Die Abschlussaktion und die Warnfrist werden beim Start zusätzlich in `active-run.json` eingefroren. Damit kann ein Klick im Live-Fenster keinen schon gestarteten Ablauf verändern. Der Schalter und das Sekundenfeld sind deshalb während eines aktiven Nachtlaufs bewusst gesperrt. In beiden Modi läuft zuerst dieselbe gewählte Warnfrist ab. Nach der letzten Sicherheitsprüfung schreibt der Wächter den Abschluss dauerhaft und fordert Windows anschließend zum Energiesparen oder sofortigen Herunterfahren auf.

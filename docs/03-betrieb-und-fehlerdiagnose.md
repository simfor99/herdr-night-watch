...
Nach einem Windows- oder WSL-Neustart wird ein unvollständig liegen gebliebener Lauf nicht fortgesetzt. Der Wächter erkennt die geänderte Boot-ID, schreibt ein `RESET`-Ereignis mit `reason=boot_id_changed` oder `reason=missing_boot_id_marker` und entfernt die alte Warnungsdatei. Im `cancellation-history.csv` steht zusätzlich der Auslöser `system_restart`.

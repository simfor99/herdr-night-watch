...
| Live-Status | Mehrere Herdr-Agenten laufen lassen, Live-Status öffnen | Live-Zahl, großer Mond mit echter Mondphase und Hover-Hinweis sowie funktionierender Start- oder Stoppknopf sind sichtbar; grüne Stunden-, weiße Minuten- und dunkel-orange Sekundenzeiger zeigen die lokale Zeit, blasse Stundenmarken liegen im Mondhof; die Temperatur sitzt über den Zeigern mittig und bleibt bei Halbmond lesbar; Abschluss-Schalter und Sekundenfeld liegen rechts neben „Herdr jetzt“, speichern nur außerhalb eines aktiven Laufes; bei Warnfrist zeigt das Fenster einen roten Countdown; die schmale Fußzeile zeigt CPU, GPU, belegten VRAM, RAM und einen verfügbaren Grafikkarten-Wattwert oder `—`; Fenster ist beweglich und schließbar |
...
| Analoge Monduhr | Live-Status öffnen, Uhrzeit mit Windows vergleichen, Randstriche und Zeigerspitzen ansehen, direkt auf den Mond rechtsklicken und `Orangen Sekundenzeiger anzeigen` zweimal umschalten | Stunde und Minute stimmen mit Windows überein; vier breite und acht schmale Stundenmarken schweben in sehr blassem Weiß vollständig außerhalb der Mondscheibe im Hof; der Minutenzeiger reicht nahe an den Außenrand, der Stundenzeiger ragt über die Temperatur hinaus und beide enden in einer klaren Instrumentenspitze; der schmale dunkel-orange Sekundenzeiger endet in einer kurzen Pfeilspitze, verschwindet und erscheint sofort; Temperatur bleibt über allen Zeigern lesbar; die letzte Auswahl bleibt nach erneutem Öffnen erhalten |
